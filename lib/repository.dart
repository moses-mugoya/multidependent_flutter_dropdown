import 'package:drop_down_app/models/state_model.dart';

class Repository {
  List<Map> getAll() => _kenyanCounties;

  getSubByCounty(String county) => _kenyanCounties
      .map((map) => CountyModel.fromJson(map))
      .where((element) => element.name == county)
      .map((map) => map.subCounties)
      .expand((element) => element)
      .toList();

  List<String> getCounties() => _kenyanCounties
      .map((map) => CountyModel.fromJson(map))
      .map((e) => e.name)
      .toList();

  List _kenyanCounties = [
    {
      "name": "Baringo",
      "capital": "Kabarnet",
      "code": 30,
